...
function _do_build_file(target, sourcefile, opt)
    ...
    local lastmtime = depend.time_compare(os.mtime(objectfile), os.mtime(dependfile))
    ...
    if not dryrun then
        ...
        local objectdepfiles = target:values("objectdepfiles")
        ...
        if objectdepfiles then
            table.join2(dependinfo.files, objectdepfiles)
        end
