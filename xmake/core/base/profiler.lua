...
function profiler._flamegraph_handler(hooktype)
    ...
    if not _G.lastsampletime then
        _G.lastsampletime = newtime
    end
    if newtime - _G.lastsampletime > 0.01 then
        _G.lastsampletime = newtime
    else
        return
    end
    ...
    _G.perffile:print(table.concat(backtraces, ";").." "..newtime)
end
...
function profiler:start()
    ...
    elseif self:is_flamegraph() then
        _G.perffile = _G.perffile or io.open("perf.fold", "w")
